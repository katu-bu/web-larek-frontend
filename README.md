...
В данной проектной работе "Веб-ларек" используется архитектурный паттерн MVC (Model-View-Controller), адаптированный под событийно-ориентированный подход с использованием `EventEmitter`.
Компоненты MVC в проекте:
1. __Модель__ 
...
3. __Контроллер__ 
Отвечает за связь между моделью и представлением, а также управляет логикой взаимодействия компонентов. В данной проектной работе контроллер - это `EventEmitter`, который позволяет моделям и представлениям взаимодействовать через события.
...
5. __Класс__ `PageView`
Помогает управлять содержимым страницы: обновлять каталог товаров, отображать количество товаров в корзине.
При клике на иконку корзины (`.header__basket`) отправляется событие `bids:open`, которое можно использовать для открытия модального окна.
_Параметры, которые принимает конструктор класса:_
...
- `set catalog(items: HTMLElement[])` — заменяет список товаров на странице, очищая контейнер `.catalog__items` и добавляет в него новые элементы.
- `set locked(value: boolean)` — блокирует или разблокирует страницу, добавляет или удаляет класс `.page__wrapper_locked` у элемента `.page__wrapper`.
...
Класс `CatalogItemView` наследуется от `Component<RenderInput>`, где `RenderInput` — дженерик, который определяет структуру данных, используемых для отображения товара (`id: string`, `title: string`, `description: string`, `image: string`, `category: string`, `price: number`).

7. __Класс__ `IconBasketView`
...
_Параметры, которые принимает конструктор класса:_
- `container` — это HTML-элемент (`HTMLElement`, содержащий кнопку корзины (`.header__basket`)), с которым работает класс.
- `events` — объект событий, реализующий интерфейс `IEvents`, для работы с событиями в классе.
_Методы класса:_
- `render(data: RenderInput): HTMLElement` — обновляет счетчик товаров в корзине.
Класс `IconBasketView` наследуется от `Component<RenderInput>`, где `RenderInput` — дженерик, который определяет структуру данных для обновления счетчика товаров (`quantity: number`).

8. __Класс__ `OrderFormView`
...
- `events` — объект событий, реализующий интерфейс `IEvents`, для работы с событиями в классе.
_Методы класса:_
- `render(_data: RenderInput): HTMLElement` — обновляет форму на основе переданных данных.
...
- `events` — объект событий, реализующий интерфейс `IEvents`, для работы с событиями в классе.
_Методы класса:_
- `render(_data: RenderInput)` — обновляет форму на основе переданных данных.
...
## Список событий
- `basket:change` — вызывается, когда товар добавляется или удаляется из корзины. В обработчик передаются обновленный список товаров и итоговая сумма.
...
- `ui:basket-open` — вызывается, когда пользователь нажимает на иконку корзины (`IconBasketView`). Используется для открытия корзины.
- `bids:open` — вызывается, когда пользователь открывает корзину с главной страницы `PageView`.
