...
- `handleFormErrors(formErrors: Partial<Record<keyof IOrderFormData, string>>)` — обрабатывает ошибки валидации формы, где `formErrors` - это объект, содержащий сообщения об ошибках для каждого поля формы.
...
- `makeAddressRequired()` — делает поле адреса обязательным для заполнения.
  Класс `OrderFormView` наследуется от `Form<IOrderFormData>`, где `IOrderFormData` — дженерик, который определяет структуру данных формы заказа (`payment?: PaymentMethod`, `address?: string`).

3. **Класс** `ContactsFormView`
...
- `handleFormErrors(formErrors: Partial<Record<keyof IContactsFormData, string>>)` — обрабатывает ошибки валидации формы, где `formErrors` - это объект, содержащий сообщения об ошибках для каждого поля формы.
  Метод устанавливает свойство `valid` в зависимости от наличия ошибок - если ошибок нет, то активируется кнопка отправки.
...
- `makePhoneRequired()` — делает поле телефона обязательным для заполнения.
  Класс `ContactsFormView` наследуется от `Form<IContactsFormData>`, где `IContactsFormData` — дженерик, который определяет структуру данных формы контактов (`email?: string`, `phone?: string`).
...
- `events` — объект событий, реализующий интерфейс `IEvents`, для работы с событиями в классе.
  _Методы класса:_
  Для установки данных в разметку используются сеттеры.
- `set itemsList(items: HTMLElement[])` — заменяет содержимое списка `.basket__list` переданными элементами, обновляя список товаров в корзине.
- `set totalPrice(totalPrice: number)` — устанавливает итоговую цену в `.basket__price`. Если цена отсутствует, поле очищается, а кнопка оформления заказа блокируется.  
  Класс `BasketView` наследуется от `Component<IBasket>`, где `IBasket` — дженерик, который определяет структуру данных, используемых для отображения корзины (`items: HTMLElement[]` и `totalPrice: number`).

6. **Класс** `BasketItemView`
...
- `events` — объект событий, реализующий интерфейс `IEvents`, для работы с событиями в классе.
  _Методы класса:_
  Для установки данных в разметку используются сеттеры.
- `set id(id: string)` — сохраняет идентификатор товара в приватное поле `_id`.
- `set title(title: string)` — устанавливает название товара в элемент `.card__title`.
- `set price(price: number | null)` — устанавливает цену в `.card__price`. Если цена отсутствует, поле очищается.
- `set index(index: number)` — устанавливает порядковый номер товара в `.basket__item-index`.
  Класс `BasketItemView` наследуется от `Component<IBasketItem>`, где `IBasketItem` — дженерик, который определяет структуру данных, используемых для отображения карточки товара в корзине (`id: string`, `title: string`, `index: number` и `price: number | null`).
...
8. **Класс** `CatalogPreviewItemView`
...
- `set id(id: string)` — сохраняет идентификатор товара в приватное поле `_id`.
- `set title(title: string)` — устанавливает название товара в элемент `.card__title`.
- `set description(description: string)` — добавляет описание товара в элемент `.card__text`.
- `set image(image: string)` — задает путь к изображению товара элемента `.card__image`.
- `set category(category: string)` — записывает категорию товара в `.card__category` и добавляет CSS-класс, определяющий цвет категории.
- `set price(price: number | null)` — устанавливает цену в `.card__price`. Если цена отсутствует, поле очищается, а кнопка добавления в корзину блокируется. То есть кнопка добавления товара в корзину для Бесценного товара всегда заблокирована.
- `set alreadyInBasket(alreadyInBasket: boolean)` — если товар уже находится в корзине, то кнопка "Добавить в корзину" деактивируется.
  Класс `CatalogPreviewItemView` наследуется от `Component<IPreviewProduct>`, где `IPreviewProduct` — дженерик, который определяет структуру данных, используемых для отображения товара (`IProduct` и `alreadyInBasket: boolean`).

9. **Класс** `CatalogItemView`  
...
- `set category(category: string)` — записывает категорию товара в `.card__category` и добавляет CSS-класс, определяющий цвет категории.
- `set price(price: number | null)` — устанавливает цену в `.card__price`. Если цена отсутствует, поле очищается.  
  Класс `CatalogItemView` наследуется от `Component<IPartialProduct>`, где `IPartialProduct` — дженерик, который определяет структуру данных для карточки товара (`id: string`, `title: string`, `image: string`, `category: string`, `price: number | null`).

10. **Класс** `OrderResultView`
...
_Методы класса:_
Для установки данных в разметку используются сеттер

- `set totalPrice(totalPrice: number)` — устанавливает итоговую цену в `.order-success__description` (пример: 'Списано 10 синапсов').
  Класс `OrderResultView` наследуется от `Component<ITotalPrice>`, где `ITotalPrice` — дженерик, который определяет структуру передаваемых данных (`totalPrice: number`).
